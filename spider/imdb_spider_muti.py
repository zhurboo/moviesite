...
def imdb_spider(url, review_url, spider_dic, id):
	...
	with open('movie_info/'+ id + '/' + id + '.txt', 'wt') as f:
		f.write(movie_json)
	...
	try:
		...
		with open('movie_info/' + id + '/'  +id + '_cover.png', 'wb') as f:
...
def imdb_spider_process(mvdic, i):
	spider_dic = {}
	...
	os.mkdir('movie_info/' + mvdic[str(i)]['id'])
	ans = imdb_spider(url, review_url, spider_dic, mvdic[str(i)]['id'])
	if ans == -1:
		print('movie id %s fails!' %(mvdic[str(i)]['id']), flush=True)
		sys.stdout.flush()
	else:
		print('movie id %s succeeds!' %(mvdic[str(i)]['id']), flush=True)
		sys.stdout.flush()
...
if __name__ == '__main__':
	...
	for i in range(len(keyList)):
		p.apply_async(imdb_spider_process, args=(mvdic, keyList[i]))
	p.close()
	p.join()
